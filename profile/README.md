...
| Hosted MCP | [`crawlora-mcp@1.17.0`](https://www.npmjs.com/package/crawlora-mcp) | 2221 tools across 260 platform groups |
| Agent Skills | [`crawlora-skills@1.23.0`](https://github.com/Crawlora-org/crawlora-skills) | REST-based catalog and focused research skills, including local competitive landscapes |
...
| --- | --- | --- |
| Go | [`crawlora-go-sdk`](https://github.com/Crawlora-org/crawlora-go-sdk) | `latest` for current SDK version `v1.41.0-sdk.1` |
| TypeScript / JavaScript | [`crawlora-typescript-sdk`](https://github.com/Crawlora-org/crawlora-typescript-sdk) | `latest` for current SDK version `v1.41.0-sdk.1` / `@crawlora-org/sdk@1.41.0-sdk.1` on npm |
...
| Ruby | [`crawlora-ruby-sdk`](https://github.com/Crawlora-org/crawlora-ruby-sdk) | `v1.41.0-sdk.1` / `latest` — gem [`crawlora`](https://rubygems.org/gems/crawlora) on RubyGems (and GitHub Packages) |
| Java / JVM | [`crawlora-java-sdk`](https://github.com/Crawlora-org/crawlora-java-sdk) | `v1.41.0-sdk.1` / `latest` — [`net.crawlora:crawlora-sdk`](https://central.sonatype.com/artifact/net.crawlora/crawlora-sdk) on Maven Central (and GitHub Packages) |
| PHP | [`crawlora-php-sdk`](https://github.com/Crawlora-org/crawlora-php-sdk) | `v1.41.0-beta.1` — [`crawlora/sdk`](https://packagist.org/packages/crawlora/sdk) on Packagist (`^1.41@beta`) |
...
For reproducible installs, pin `v1.41.0-sdk.1` for Git-based SDKs and
`@crawlora-org/sdk@1.41.0-sdk.1` for TypeScript.
...
composer require crawlora/sdk:^1.41@beta
```
...
`@crawlora-org/sdk`. Python is published to PyPI as `crawlora` (a `1.41.0.dev1`
...
`https://mcp.crawlora.net/mcp` exposes the public API as 2221 MCP tools using
stable `family.action` names.

| Integration | Repository | What it is |
| --- | --- | --- |
| MCP server | [`crawlora-mcp`](https://github.com/Crawlora-org/crawlora-mcp) | Version `1.17.0`. Hosted (and local stdio) Model Context Protocol server exposing the public API as 2221 MCP tools. Connect any MCP client to `https://mcp.crawlora.net/mcp`. |
| Agent Skills | [`crawlora-skills`](https://github.com/Crawlora-org/crawlora-skills) | Version `1.23.0`. 112 installable skills, including 74 bundled installable [Agent Skills](https://docs.claude.com/en/docs/agents-and-tools/agent-skills/overview) (`SKILL.md` packages) plus per-platform skills that teach any coding agent — Claude Code, Codex, Cursor, Copilot — how to fetch structured web data over the REST API. This release expands publisher-native news coverage and adds a collectibles-market workflow for PSA references, listings, auctions, sold items, and market trends. No MCP setup required. Also a Claude Code plugin marketplace. |
